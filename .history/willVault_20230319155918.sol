...
*/
	...
	function addNonTestator(address _addr, string memory _name, uint _userType) public onlyTestator {
		...
		testators[testators.length - 1].nonTestators[getNonTestatorCount(testators.length - 1)] = newNonTestator;
...
function getBeneficiaries() public view onlyTestator  returns (NonTestator[] memory) {
    ...
    for (uint i = 0; i < count; i++) {
        if (testators[getTestatorIndex(msg.sender)].nonTestators[i].userType == NonTestatorType.Beneficiary) {
            beneficiaries[index] = testators[getTestatorIndex(msg.sender)].nonTestators[i];
            index++;
        }
    }
...
//     for (uint i = 0; i < nonTestators.length; i++) {
//         if (nonTestators[i].addr == msg.sender && nonTestators[i].password == passwordHash && nonTestators[i].userType == NonTestatorType.Beneficiary) {
//             nonTestators[i].isLogin = true;
//             break;
//         }
//     }
